<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="serverUri" :placeholder="$t('monitor.columns.executorUri')" class="filter-item" style="width: 260px;" @change="change">
...
<script>
import { listExecutorServiceReq, getServerInfoReq } from '@/api/core/tool'
    ...
    loadService() {
      listExecutorServiceReq().then(res => {
        if (!res.data.result || res.data.result.length === 0) {
          this.$message.warning(this.$t('tip.noneExecutor'))
        } else {
