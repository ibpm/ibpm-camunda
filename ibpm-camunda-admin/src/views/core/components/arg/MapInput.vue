<template>
  <div>
    <el-row v-for="(item, index) in options" :key="index" style="margin-bottom: 2px;">
      ...
      <el-col :span="7">
        <string-input v-model="item.key" :placeholder="$t('core.arg.placeholder.mapKey')" />
      </el-col>
      <el-col :span="8" :offset="1">
        <string-input v-if="genericType === 0" v-model="item.val" :placeholder="$t('core.arg.placeholder.mapValue')" />
        ...
        <string-input v-else v-model="item.val" :placeholder="$t('core.arg.placeholder.argValue')" />
      </el-col>
    </el-row>
  </div>
</template>
  ...
  methods: {
    initValue(value) {
      ...
      if (value) {
        ...
        list.forEach(item => {
          if (item && item.length > alias.colon.length) {
            const keyValArr = item.split(alias.colon)
            if (!isBlank(keyValArr[0]) && keyValArr.length === 2) {
              arr.push({
                key: keyValArr[0],
                val: keyValArr[1]
              })
    ...
    change(opts) {
      ...
      if (opts) {
        opts.forEach(item => {
          if (!isBlank(item.key) && !isBlank(item.val)) {
            result += item.key + alias.colon + item.val + alias.comma
          }
        })
      }
