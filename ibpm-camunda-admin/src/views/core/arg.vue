<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="page.argName" :placeholder="$t('core.arg.columns.argName')" class="filter-item search-input" @keyup.enter.native="search" />
      <el-select v-model="page.argType" :placeholder="$t('core.arg.columns.argType')" clearable collapse-tags class="filter-item search-select">
        <el-option v-for="item in argTypes" :key="item.value" :label="item.label" :value="item.value" />
      </el-select>
      <el-input v-model="page.argValue" :placeholder="$t('core.arg.columns.argValue')" class="filter-item search-input" @keyup.enter.native="search" />
    ...
    >
      <el-table-column
        ...
        width="45"
      />
      <el-table-column :label="$t('core.arg.columns.argName')" prop="id" sortable="custom" min-width="100px">
    ...
    <el-dialog :title="editDialog.title" :visible.sync="editDialog.base.visible" :center="true" :modal="true" :close-on-click-modal="false" :close-on-press-escape="false" :before-close="close">
      <div v-show="editDialog.base.visible">
        <el-form ref="form" :rules="formRules" :model="arg" label-position="right" label-width="150px" class="form-layout">
          <el-form-item :label="$t('core.arg.columns.argName')" prop="argName">
            <el-input v-model="arg.argName" :placeholder="$t('core.arg.placeholder.argName')" />
          </el-form-item>
          <el-form-item :label="$t('core.arg.columns.argType')">
            <el-row>
              <el-col :span="isRaw || isTradeDate ? 12 : 24">
              ...
              <el-col :span="11" :offset="1">
                <el-select v-if="isRaw" v-model="arg.genericType" :placeholder="$t('core.arg.placeholder.genericType')">
                  <el-option v-for="item in genericTypes" :key="item.label" :value="item.value" :label="item.label" />
                </el-select>
                <el-select v-if="isTradeDate" v-model="calendarName" :placeholder="$t('core.arg.placeholder.calendarName')" clearable filterable>
                  <el-option v-for="item in calendars" :key="item.calendarName" :value="item.calendarName" :label="item.displayName" />
                </el-select>
              </el-col>
            </el-row>
          </el-form-item>
          <el-form-item :label="$t('core.arg.columns.argValue')" prop="argValue">
            <string-input v-if="arg.argType === 0" v-model="arg.argValue" :placeholder="$t('core.arg.placeholder.argValue')" />
            ...
            <string-input v-else v-model="arg.argValue" :placeholder="$t('core.arg.placeholder.argValue')" />
          </el-form-item>
...
<script>
import { listReq, removeReq, addReq, updateReq, getTradeDateReq } from '@/api/core/arg'
import * as calendarApi from '@/api/core/calendar'
import { argTypesReq, tradeDateUnitsReq } from '@/api/core/tool'
...
export default {
  ...
  methods: {
    setFormRules() {
      this.formRules.argName[0].message = this.$t('core.arg.rules.argName')
      this.formRules.argValue[0].message = this.$t('core.arg.rules.argValue')
    },
    search() {
