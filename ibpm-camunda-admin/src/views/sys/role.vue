    ...
    >
      ...
      <el-table-column :label="$t('actions.handle')" align="center" width="260px" class-name="small-padding">
        <template slot-scope="scope">
          ...
          <el-button type="success" @click="openUserDialog(scope.row)">{{ $t('sys.role.actions.user') }}</el-button>
    ...
    <el-dialog :title="editDialog.title" :visible.sync="editDialog.base.visible || editDialog.user.visible || editDialog.resource.visible" :center="true" :modal="true" :close-on-click-modal="false" :close-on-press-escape="false" :before-close="close">
      ...
      <div v-show="editDialog.user.visible" class="custom-transfer">
        <el-transfer
          ...
          :data="transferData"
          :filter-placeholder="$t('sys.role.tip.userFilterTip')"
...
<script>
import { listReq, getReq, removeReq, addReq, updateReq, getUsersReq, updateRoleUserReq, getResourcesReq, updateRoleResourceReq } from '@/api/sys/role'
...
export default {
  ...
  methods: {
    ...
    openUserDialog(row) {
      ...
      this.editDialog.title = this.$t('sys.role.tip.user')
      this.selectRow(row)
      this.generateData()
    },
    ...
    generateData() {
      ...
      getUsersReq({ roleName: this.selections[0].roleName }).then(res => {
    ...
    handleChange(value, direction, movedKeys) {
      ...
      updateRoleUserReq(data).then(res => {
        this.$message.success(res.data.msg)
      })
    },
    renderFunc(h, option) {
