<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-access="'user-add'" class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-plus" @click="update(null)" />
      <el-button v-access="'user-remove'" :disabled="!selections || !selections.length" class="filter-item" style="margin-left: 10px;" type="danger" icon="el-icon-delete" @click="remove" />
    </div>
    ...
    >
      ...
      <el-table-column :label="$t('sys.user.columns.email')" min-width="100px">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <p>{{ scope.row.email }}</p>
            <div slot="reference">
      ...
      <el-table-column :label="$t('sys.user.columns.mobile')" min-width="100px">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
      ...
      <div v-show="editDialog.base.visible">
        <el-form ref="form" :rules="formRules" :model="user" label-position="right" label-width="150px" class="form-layout">
          <el-form-item v-show="editDialog.oper === 'add'" :label="$t('columns.userName')" prop="userName">
            <el-input v-model="user.userName" :placeholder="$t('placeholders.userName')" />
          </el-form-item>
          <el-form-item :label="$t('columns.displayName')" prop="displayName">
            <el-input v-model="user.displayName" :placeholder="$t('placeholders.displayName')" />
          </el-form-item>
          <el-form-item :label="$t('columns.password')" prop="password">
            <el-input v-model="user.password" :placeholder="$t('placeholders.password')" :type="passwordType"><svg-icon slot="suffix" :icon-class="passwordType === 'password' ? 'eye' : 'eye-open'" @click="passwordType = (passwordType === 'password' ? '' : 'password')" /></el-input>
          </el-form-item>
          <el-form-item :label="$t('columns.status')">
            <el-select v-model="user.status" style="width: 100%;">
              <el-option v-for="item in userStatuses" :key="item.label" :value="item.value" :label="item.label" />
            </el-select>
            <el-alert v-if="user.status !== 0" style="padding: 0;" :type="user.status === 1 ? 'warning' : 'error'" :title="user.status === 1 ? $t('tip.userFrozen') : $t('tip.userDisabled')" />
          </el-form-item>
          ...
          <el-form-item :label="$t('sys.user.columns.sendConfig')">
...
<script>
import { listReq, getReq, removeReq, addReq, updateReq, updatePasswordReq } from '@/api/sys/user'
...
export default {
  ...
  data() {
    return {
      ...
      selections: [],
      configs: [],
      canUpdate: canAccess('user-update')
    }
  },
  created() {
    this.loadConst()
  ...
  methods: {
    setFormRules() {
      ...
      this.passwordFormRules.oldPassword[0].message = this.$t('sys.user.rules.oldPassword')
      this.passwordFormRules.newPassword[0].message = this.$t('sys.user.rules.newPassword')
      this.passwordFormRules.doubleNewPassword[0].message = this.$t('sys.user.rules.doubleNewPassword')
    },
    search() {
    ...
    reset() {
      if (!this.selections || !this.selections.length || !this.selections[0].userName) {
        this.user = Object.assign({}, DEF_OBJ)
      } else {
        getReq({ userName: this.selections[0].userName }).then(res => {
          this.user = res.data.result
          this.configs = []
          if (this.user.sendConfig) {
            for (let i = 0; i < this.sendConfigs.length; i++) {
              if (this.user.sendConfig.length > i && this.user.sendConfig[i] !== '0') {
    ...
    save() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.user.sendConfig = ''
          for (let i = 0; i < this.sendConfigs.length; i++) {
            if (this.configs.find(item => { return item === this.sendConfigs[i] })) {
              this.user.sendConfig += '1'
            } else {
              this.user.sendConfig += '0'
            }
          }
    ...
    remove() {
      ...
      const adminUser = this.selections.find(item => item.superAdmin > 0)
      if (adminUser) {
        this.$message.warning(this.$t('tip.adminUserCannotBeRemoved') + ':' + adminUser.userName)
        return
      }
    ...
    formatStatus(item) {
      for (let i = 0; i < this.userStatuses.length; i++) {
        const option = this.userStatuses[i]
    ...
    loadConst() {
      ...
      import('@/lang/dict.js').then(array => {
        this.userStatuses = array['userStatuses_' + lang]
